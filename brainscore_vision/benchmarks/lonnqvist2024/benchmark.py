...
class _Lonnqvist2024IndividualCurvesEngineeringAccuracy(_Lonnqvist2024Base):
    def __init__(self, condition: int):
        metric = load_metric('accuracy')
        ceiling_func = lambda: Score(1)
        super(_Lonnqvist2024IndividualCurvesEngineeringAccuracy, self).__init__(
            identifier='Lonnqvist2024-engineering_accuracy', dataset='inlab-instructions',
            ceiling_func=ceiling_func,
            metric=metric)

        self._stimulus_set = self._stimulus_set[self._stimulus_set['curve_length'] == condition]
